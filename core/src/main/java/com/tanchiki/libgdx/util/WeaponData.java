package com.tanchiki.libgdx.util;

public class WeaponData {
    public static int light_bullet = 100;

    public static int double_light_bullet = 0;

    public static int double_palzma = 0;

    public static int bronet_bullet = 0;

    public static int bronet_bullet2 = 0;

    public static int art = 0;

    public static int mine1 = 0;

    public static int plazma = 0;

    public static int mine2 = 0;

    public static int rocket = 0;

    public static int tnt1 = 0;

    public static int tnt2 = 0;

    public static int tnt3 = 0;

    public static int time = 0;

    public static int speed = 0;

    public static int live = 4;

    public static int fix = 0;

    public static int air = 0;

    public static int shield = 0;

    public static int modern_tank = 0;

    public static int radar = 0;

    public static int termovisor = 0;

    public static int tank1 = 0;

    public static int tank2 = 0;

    public static int tank3 = 0;

    public static int brone1 = 0;

    public static int brone2 = 0;

    public static class Upgrade {
        public static int bullet = 0;

        public static int rocket = 0;

        public static int air = 0;

        public static int mine = 0;

        public static int tnt = 0;

        public static int art = 0;

        public static int shield = 0;

        public static int brone = 0;

        public static int tank_unity = 0;

        public static int adding_percent = 0;
    }

    public static class Type {
        public static final int light_bullet = 4;

        public static final int double_light_bullet = 6;

        public static final int double_plazma_bullet = 7;

        public static final int bronet_bullet = 15;

        public static final int bronet_bullet2 = 16;

        public static final int art = 9;

        public static final int mine1 = 10;

        public static final int plazma = 5;

        public static final int mine2 = 18;

        public static final int rocket = 8;

        public static final int tnt1 = 19;

        public static final int tnt2 = 11;

        public static final int tnt3 = 20;

        public static final int time = 12;

        public static final int speed = 13;

        public static final int live = 27;

        public static final int fix = 26;

        public static final int air = 22;

        public static final int shield = 23;

        public static final int brone1 = 25;

        public static final int brone2 = 24;

        public static final int radar = 28;

        public static final int termovisor = 29;

        public static final int tank1 = -1;

        public static final int tank2 = -2;

        public static final int tank3 = -3;

        public static final int modern_tank = -4;
    }
}
