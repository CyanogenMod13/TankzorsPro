...
public class Bonus extends GameActor {
    ...
    public Bonus(int[] index, int type_bonus, float x, float y) {
        ...
        t = GameStage.getInstance().TextureLoader.getBonus()[0];
			...
			switch (type_bonus) {
				...
				default: if (tank instanceof TankUser) {
					...
					switch (MainTerrain.getCurrentTerrain().getParameters().getKey(123 + type_bonus - 1)) {
						case 0: PanelStage.getInstance().addToast("Подстава, пустой ящичек!"); break;
						case 1: PanelStage.getInstance().addToast("Снаряды — 30 зарядов"); WeaponData.light_bullet += 30; break;
						case 2: PanelStage.getInstance().addToast("Плазма — 20 зарядов"); WeaponData.plazma += 20; break;
						case 3: PanelStage.getInstance().addToast("Двойная плазма — 20 зарядов"); WeaponData.double_palzma += 20; break;
						case 4: PanelStage.getInstance().addToast("Двойные снаряды — 20 зарядов"); WeaponData.double_light_bullet += 20; break;
						case 5: PanelStage.getInstance().addToast("Бронебойные обычные — 10 зарядов"); WeaponData.bronet_bullet += 10; break;
						case 6: PanelStage.getInstance().addToast("Бронебойные усиленные — 10 зарядов"); WeaponData.bronet_bullet2 += 10; break;
						case 7: PanelStage.getInstance().addToast("Мины слабые — 10 штук"); WeaponData.mine1 += 10; break;
						case 8: PanelStage.getInstance().addToast("Мины мощные — 5 штуки"); WeaponData.mine2 += 5; break;
						case 9: PanelStage.getInstance().addToast("Динамит первого типа — 3 штуки"); WeaponData.tnt1 += 3; break;
						case 10: PanelStage.getInstance().addToast("Динамит второго типа — 2 штуки"); WeaponData.tnt2 += 2; break;
						case 11: PanelStage.getInstance().addToast("Динамит третьего типа — 1 штука"); WeaponData.tnt3 += 1; break;
						case 12: PanelStage.getInstance().addToast("Артиллерия — 2 штуки"); WeaponData.art += 2; break;
						case 13: 
						case 14: PanelStage.getInstance().addToast("Боеголовки — 2 штуки"); WeaponData.rocket += 2; break;
						case 15: PanelStage.getInstance().addToast("Поддержка с воздуха — 1 штука"); WeaponData.air += 1; break;
						case 16: PanelStage.getInstance().addToast("Союзник — 1 штука"); WeaponData.tank3 += 1; break;
						case 17: PanelStage.getInstance().addToast("Силовое поле — 3 единицы"); WeaponData.shield += 3; break;
						case 18: PanelStage.getInstance().addToast("Временная броня — 1 единица"); WeaponData.brone1 += 1; GameStage.TankUser.HP++; break;
						case 19: PanelStage.getInstance().addToast("Аааа! Allah Akbar!"); MainTerrain.getCurrentTerrain().decor_ground.addActor(new TNT2(getCenterX(), getCenterY())); break;
					}
					remove();
				}
			}
