...
public class MiniMap extends Actor {
    ...
    public void drawDebug(ShapeRenderer shapes) {
        ...
        for (int i = 0; i < ground.size; i++) {
            Actor a = (Actor) ground.get(i);
            if (a instanceof Grass)
                shapes.setColor(colorGrass);
            if (a instanceof Sand)
                shapes.setColor(colorSand);

            shapes.rect(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size, size, size);
        }

        Array<Actor> bonus = m.bonus.getChildren();
        if (render > 10 && WeaponData.termovisor >= 2)
            for (int i = 0; i < bonus.size; i++) {
                Actor a = (Actor) bonus.get(i);
                shapes.setColor(Color.SKY);

                shapes.rect(getX() + a.getX(Align.center) / 2 * (size) + size / 2 - (size / 1.5f / 2), getY() + a.getY(Align.center) / 2 * (size) + size / 2 - (size / 1.5f / 2), size / 1.5f, size / 1.5f);
            }

        for (int i = 0; i < m.walls.getChildren().size; i++) {
            Actor a = (Actor) m.walls.getChildren().get(i);
            ...
            if (a instanceof SmallTree || a instanceof Cactus) {
                ...
                shapes.circle(getX() + a.getX(Align.center) / 2 * size + size / 2, getY() + a.getY(Align.center) / 2 * size + size / 2, size / 2);
            } else shapes.rect(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size, size, size);
        }

        for (int i = 0; i < m.builds.getChildren().size; i++) {
            Actor a = (Actor) m.builds.getChildren().get(i);
            ...
            if (a instanceof RedFlag) {
                ...
                shapes.triangle(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size,
                        getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size + size,
                        getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size + size);
            }
        ...
        if (WeaponData.termovisor >= 1) {
            for (int i = 0; i < m.tanks_enemy.getChildren().size; i++) {
                Actor a = (Actor) m.tanks_enemy.getChildren().get(i);
                shapes.setColor(Color.RED);
                //shapes.rect(getX() + a.getX(Align.center)/2 * size,getY() + a.getY(Align.center)/2 * size,size,size);
            ...
            for (int i = 0; i < m.tanks_unity.getChildren().size; i++) {
                Actor a = (Actor) m.tanks_unity.getChildren().get(i);
    ...
    private void printTanks(ShapeRenderer shapes, Actor a, float size) {
        switch (((Tank) a).getDirection()) {
            case Tank.DOWN:
                shapes.triangle(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size + size,
                        getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size + size,
                        getX() + a.getX(Align.center) / 2 * size + size / 2, getY() + a.getY(Align.center) / 2 * size);
                break;
            case Tank.RIGHT:
                shapes.triangle(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size,
                        getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size + size,
                        getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size + size / 2);
                break;
            case Tank.LEFT:
                shapes.triangle(getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size,
                        getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size + size,
                        getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size + size / 2);
                break;
            case Tank.UP:
                shapes.triangle(getX() + a.getX(Align.center) / 2 * size, getY() + a.getY(Align.center) / 2 * size,
                        getX() + a.getX(Align.center) / 2 * size + size, getY() + a.getY(Align.center) / 2 * size,
                        getX() + a.getX(Align.center) / 2 * size + size / 2, getY() + a.getY(Align.center) / 2 * size + size);
                break;
        }
    }
}
